alter table public.sales_records
  ...
  add column if not exists customer_middle_name text,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sales_records_customer_first_name_check'
  ) then
    alter table public.sales_records
      add constraint sales_records_customer_first_name_check
      check (
        customer_first_name is null
        or nullif(btrim(customer_first_name), '') is not null
      );
  end if;
end
$$;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sales_records_customer_last_name_check'
  ) then
    alter table public.sales_records
      add constraint sales_records_customer_last_name_check
      check (
        customer_last_name is null
        or nullif(btrim(customer_last_name), '') is not null
      );
  end if;
end
$$;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sales_records_customer_address_check'
  ) then
    alter table public.sales_records
      add constraint sales_records_customer_address_check
      check (
        customer_address is null
        or nullif(btrim(customer_address), '') is not null
      );
  end if;
end
$$;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sales_records_customer_email_check'
  ) then
    alter table public.sales_records
      add constraint sales_records_customer_email_check
      check (
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sales_records_customer_phone_check'
  ) then
    alter table public.sales_records
      add constraint sales_records_customer_phone_check
      check (
...
comment on column public.sales_records.customer_first_name is 'Customer first name captured at checkout.';
comment on column public.sales_records.customer_middle_name is 'Customer middle name captured at checkout (optional).';
comment on column public.sales_records.customer_last_name is 'Customer last name captured at checkout.';
comment on column public.sales_records.customer_address is 'Customer delivery or contact address captured at checkout.';
comment on column public.sales_records.customer_email is 'Customer email address used for order updates.';
comment on column public.sales_records.customer_phone is 'Customer mobile number used for order updates.';
...
create or replace function public.record_sale_and_adjust_inventory(
  ...
  p_customer_middle_name text default null,
...
begin
  ...
  v_customer_first_name := nullif(btrim(p_customer_first_name), '');
  v_customer_middle_name := nullif(btrim(p_customer_middle_name), '');
  v_customer_last_name := nullif(btrim(p_customer_last_name), '');
  v_customer_address := nullif(btrim(p_customer_address), '');
  v_customer_email := nullif(lower(btrim(p_customer_email)), '');
  v_customer_phone := nullif(btrim(p_customer_phone), '');
  ...
  on conflict (invoice_number) do update
    set
      ...
      customer_middle_name = excluded.customer_middle_name,
...
comment on function public.record_sale_and_adjust_inventory(text, timestamptz, numeric, jsonb, text, text, text, text, text, text) is
  'Creates/updates sales record with customer contact info, stores normalized items, and attempts inventory deduction without blocking checkout.';
